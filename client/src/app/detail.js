...
import { Heading, Blockie, Flex, QR, Button, Text, Box, Loader, Card, Modal, Input } from 'rimble-ui';
...
import WETStore from './stores/wet.store';
...
const Detail =  observer((props) => {
    ...
    const updateDetailWETState = async () => {
        const accumulated = new BN(
            await accumulatedForIndex(props.detailId),
          );
          detailStore.owner = await getWETOwner(props.detailId);
          detailStore.name = (await getWETName(props.detailId)) || 'Unnamed';
          detailStore.accumulatedWET = Number(await toEthUnit(accumulated)).toFixed(2);
          detailStore.wetBalance = Number(await toEthUnit(await wetBalanceOf())).toFixed( 2, );
    }
    ...
    const listTrait = (trait) => {
        if(trait.value)
        {
            return <Button.Outline key={trait.trait_type} className="trait-buttons" title={trait.trait_type} disabled={true}>
                {trait.value}
            </Button.Outline>
        ...
        <Flex>
                        ...
                        <Box className="waifu-card-box-pink-sub waifu-pic-box waifu-card-sub-black" color="white" style={{maxWidth:1424, marginBottom:1}}>
                            <center className="waifu-pic-box-center waifu-card-pink-center">
                                ...
                                <img className="waifu-dotted-line" src={WaifuDottedLine}/>
                                    ...
                                    <img className="waifu-card-image" src={GLOBALS.WAIFUS_VIEWABLE_URL==''?GLOBALS.DEFAULT_WAIFU_IMAGE : GLOBALS.WAIFUS_VIEWABLE_URL +"/" + revealedWaifuIndex(props.detailId) +'.png'}/>
                                    <br/>
                                ...
                                <span className="card-box-center-pink-text test-margin">
                                        ...
                                        <span className="waifu-box-number"> {props.detailId} / {GLOBALS.TOTAL_WAIFUS}</span>
                                    </span>
                            </center>
                        </Box>
                    </div>
            ...
            <Box className="waifu-card-box waifu-card waifu-detail-box-sub waifu-detail-box-sub-container" >
                <div className="box-upper waifu-detail-box-sub">
                    ...
                    <Box className="waifu-card-box-sub waifu-detail-box-sub waifu-detail-box-sub" color="white">
                        <center className="waifu-card-box-center">
                        {detailStore.currentWaifu != null &&
                            <Box>
                                <Flex className="waifu-wet-rules" flexWrap="wrap" style={{maxwidth:'590px'}}>
                                    {detailStore.currentWaifu.traits.map( trait => listTrait(trait))}
                                </Flex>
                            </Box>
                        }
                        <Box>
                        <div style={{marginTop:'55px'}}>
                            <Text className="waifu-detail-text">Current WET: {detailStore.wetBalance}</Text>
                            <Text className="waifu-detail-text">Accumulated WET: {detailStore.accumulatedWET}</Text>
                            ...
                            {walletStore.defaultAddress == detailStore.owner  &&
                                ...
                                {!detailStore.isApproved && 
                                    ...
                                    {detailStore.isApproved &&
                                        <center>
                                    <Box className="wallet-buttons">
                                        <Button.Outline className="waifu-card-buttons" onClick={openNCModal}><span className="waifu-button-learnmore" > Change Name</span> </Button.Outline>
                                    </Box>
                                    </center>
                                    }
                                </Box>
                ...
                <Box className="waifu-card-box" >
                    <div className="box-upper">
                        ...
                        <Box className="waifu-card-box-presale-sub" color="white" style={{maxWidth:1000, marginBottom:1}}>
                            <center className="waifu-card-box-center">
                                <div className="modal-header-text">Please enter a new name</div>
                                <Input className="waifu-input waifu-input-name" type="text" placeholder="Please enter a new name" value = {detailStore.changedName} onChange={(event)=>{
                                    detailStore.updateChangedName(event.target.value);}}></Input>
                                <Text className="modal-error-text">{detailStore.nameValidation}</Text>
                                <br/>
                                <Box className="modal-button-container">
                                    <Button.Outline className="modal-buttons" onClick={closeNCModal}><span className="waifu-button-learnmore"> Cancel</span> </Button.Outline>
                                    <Button.Outline className="modal-buttons" onClick={validateChangeName}><span className="waifu-button-learnmore" > Change Name</span> </Button.Outline>
                                </Box>
            ...
            <Box className="waifu-presale-container">
                <Box className="waifu-card-box" >
                    <div className="box-upper">
                        ...
                        <Box className="waifu-card-box-presale-sub" color="white" style={{maxWidth:1000, marginBottom:1}}>
                            <center className="waifu-card-box-center">
                                <div className="modal-header-text">Insufficient Balance or Allowance</div>
                                <Text className="modal-main-text">The balance is below the required amount for a name change or there is not enough allowance/approval</Text>
                                <br/>
                                <Box className="modal-button-container">
                                    <Button.Outline className="modal-buttons" onClick={closeNCModal}><span className="waifu-button-learnmore"> Cancel</span> </Button.Outline>                                    
                                </Box>
